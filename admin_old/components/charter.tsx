...
const Charter = ({setActive}:{setActive:any}) => {
    const [charter, setCharter] = useState<ICharter[]>([]);
    const [featured, setFeatured] = useState<boolean>(false);
    useEffect(() => {
        getCharters().then((charter) => setCharter(charter));
    }, []);
    ...
    return (
        ...
        >
            <div className={"w-fit flex flex-col"}>
                ...
                <Button onClick={()=>setFeatured(!featured)} className={`mx-auto my-5 ${featured && "bg-amber-300"}`}>Featured <div className={"h-[60%]"}>
                    <svg
            ...
            >
                {charter.length > 0 && charter.map((charter, i: number) => {
                    if (featured && !charter.featured) return null;
                    return (
                        <Manage key={i} setYachts={setCharter} data={charter}/>
                    );
