...
const Card = ({
  ...
  const t = useTranslations("yachts.carousel"),
    ...
    price = data.price
      ? type === "charters"
        ...
        : formatCurrency(data.price * rates[currency], currency)
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage: `url(${encodeURI(data.photos.featured.sizes.thumbnail.url)})`,
        }}
      >
        {data.etiquette && (
          <p
      ...
      <div className={"w-full flex flex-col justify-start items-start"}>
        <div
          className={`w-full flex justify-between items-baseline text-black uppercase`}
        >
          <p>{data.name}</p>
          <p>{price}</p>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        {data.map((card, i) => (
          // @ts-ignore
          <Card key={i} data={card} type={type} />
        ))}
      </div>
      <div
        className={
