...
const Card = ({ card }: { card: IFeatured }) => {
  const t = useTranslations("destination.charters"),
  ...
  return (
    <Link
      ...
      <p className={"uppercase text-rock-400"}>
        {card.builder} | {convertUnit(card.length, units.length) + units.length} | {card.yearBuilt}{" "}
        | {card.sleeps} {t("sleeps")}
      </p>
    </Link>
  );
};
    ...
    <section
      ...
      <div className={"w-full flex justify-between items-center"}>
        ...
        <div className={"flex justify-center items-center gap-[2vw]"}>
          ...
          <Link href={"/sales"} className={"glass-button glass-button-dark whitespace-nowrap"}>
            {t("all")}
          </Link>
        </div>
      </div>
      ...
      <div className={"hidden h-max lg:flex justify-start items-baseline w-full overflow-x-clip"}>
