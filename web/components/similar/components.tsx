...
const Card = ({
    ...
    let price;
    if (type === "charters") {
        price = `${formatCurrency((card as ICFeatured).price.low * rates[currency], currency)} - ${formatCurrency((card as ICFeatured).price.high * rates[currency], currency)}`;
    } else {
        price = formatCurrency((card as IFeatured).price * rates[currency], currency);
    }
    switch (type) {
        case "yachts":
            link = "/sales";
    ...
    return (
        <Link
            href={{ pathname: `${link}/[id]` as any, params: { id: card.id } }}
            ...
            <div
                ...
                {price ? (
                    ...
                    <div className={"bg-rock-200 lg:w-[10vw] h-[1rem] rounded-full"}></div>
                )}
            </div>
...
const CarouselButton = ({ direction, onClick }: { direction: string; onClick: () => void }) => {
    return (
        <button
            type={"button"}
            onClick={onClick}
...
const Section = ({
    ...
    const t = useTranslations("similar"),
        ...
        setTranslate = (amount: number) => {
            document.documentElement.style.setProperty("--translate-featured", `${amount}%`);
        },
        getTranslation = () =>
            parseInt(document.documentElement.style.getPropertyValue("--translate-featured")),
        setAnimate = (duration: number) =>
            document.documentElement.style.setProperty("--animate-featured", `${duration}ms`),
        ...
        <section
            ...
            <div
                ...
                {carouselData.map((card, i) => (
                    <Card key={i} card={card} type={type} />
                ))}
            </div>
            <div
                className={
