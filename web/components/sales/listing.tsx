...
interface IYacht
    extends Pick<
        Yacht,
        "name" | "category" | "price" | "builder" | "length" | "yearBuilt" | "sleeps" | "photos"
    > {
    _id: string;
}
...
const Card = ({ data }: { data: IYacht }) => {
    const t = useTranslations("index.featured"),
        { currency, units, bookmarks, addBookmark, removeBookmark, rates } = useViewContext(),
            ...
            <div className={"w-full h-max lg:overflow-x-hidden overflow-x-scroll"}>
                <div
                    ...
                    <div
                        ...
                        <p>{/* TODO: Add yacht's "status" (i.e. sold, exclusive, new, ...) */}</p>
                        <button
                            type={"button"}
                            onClick={(e) => {
                                e.preventDefault();
    ...
    useEffect(() => {
        const filtered = data.filter((yacht) => {
            ...
            if (filter.length.min !== undefined && filter.length.max !== undefined) {
                if (yacht.length < filter.length.min || yacht.length > filter.length.max)
                    return false;
            }
            if (filter.builder && yacht.builder !== filter.builder) return false;
            if (filter.price.min !== undefined && filter.price.max !== undefined) {
                if (yacht.price < filter.price.min || yacht.price > filter.price.max) return false;
            }
            if (filter.sleeps !== undefined && yacht.sleeps !== filter.sleeps) return false;
                ...
                <div className={"filter-column"}>
                    <Select
                        ...
                        options={[
                            ...
                            ...Array.from(new Set(data.map((yacht) => yacht.name))).map((name) => ({
                                value: name,
                                label: name,
                            })),
                        ]}
