...
export const Edit = ({ data }: { data: IBrokerino }) => {
  ...
  return (
    <section
      className={
        "containerize h-full bg-stone-100 flex flex-col justify-center items-center"
      }
    >
      <form
        className={"w-2/6 flex flex-col justify-center items-center gap-[4vh]"}
      >
        <h1>Edit your profile, {data.name}</h1>
        <div
          className={
        ...
        <div
          className={
            "w-full flex flex-col justify-start items-center gap-[2vh]"
          }
        >
          <Input
            variant={"flat"}
...
const Brokerino = ({ id }: { id: IBrokerino["kindeID"] }) => {
  const [data, setData] = useState<IBrokerino | null | undefined>(undefined);

  useEffect(() => {
    fetchBrokerino(id).then((res) => setData(res));
  }, []);
